...
class App extends Component {
  ...
  toggleVisitStatus = id => {
    this.setState(prevState => ({
      countryList: prevState.countryList.map(eachCountry => {
  ...
  render() {
    ...
    const visitedCountriesList = countryList.filter(
      ...
      <div className="AppContainer">
        ...
        <ul className="CountriesListContainer">
          {countryList.map(eachCountry => (
            <CountriesList
              key={eachCountry.id}
              countriesListDetails={eachCountry}
              toggleVisitStatus={this.toggleVisitStatus}
            />
          ))}
        </ul>
        <div className="VisitedCountriesContainer">
          <h1 className="Heading">Visited Countries</h1>
          <ul className="VisitedCountriesList">
            {visitedCountriesList.length === 0 ? (
              <>
                <p className="NotFound">No Countries Visited Yet</p>
              </>
            ) : (
              <>
                {visitedCountriesList.map(eachCountry => (
                  <VisitedCountries
                    key={eachCountry.id}
                    visitedCountriesDetails={eachCountry}
                    toggleVisitStatus={this.toggleVisitStatus}
                  />
                ))}
              </>
